...
def add_grid_variables(ds, grid_name='POP_gx1v7', grid_vars=['TAREA', 'KMT', 'REGION_MASK', 'dz'], model='pop',
                       mom_static_file='/glade/scratch/mlevy/archive/mom_hybrid_z/ocn/hist/mom_hybrid_z.mom6.static.nc',
                       mom_vertical_grid_file='/glade/scratch/mlevy/archive/mom_oob.002/ocn/hist/mom_oob.002.mom6.h_bgc_monthly_z_0014_03.nc'):
...
def global_mean(ds, normalize=True, include_ms=False, region_mask=None, model='pop'):
    ...
    compute_vars = [
        v for v in ds 
        if 'year' in ds[v].dims and horizontal_dims == ds[v].dims[-2:]
    ]
    
    other_vars = list(set(ds.variables) - set(compute_vars))

    if include_ms:
        surface_mask = ds[area_field].where(ds[land_sea_mask] > 0).fillna(0.)
    else:
        surface_mask = ds[area_field].where(ds.REGION_MASK > 0).fillna(0.)
    ...
    with xr.set_options(keep_attrs=True):
        ...
        dso = xr.Dataset({
            v: (ds[v] * masked_area[v]).sum(horizontal_dims)
            for v in compute_vars
        })
        if normalize:
            dso = xr.Dataset({
                v: dso[v] / masked_area[v].sum(horizontal_dims)
                for v in compute_vars
            })            
        else:
            for v in compute_vars:
