...
def generate_time_depth_plots(ds, variable_dict, vertical_dim='z_t', ylim=(1000, 0), levels=20, cmap='inferno', vmin=None, vmax=None, log_cbar=False):
    ...
    for variable in variable_dict.keys():
        if (vertical_dim in ds[variable].dims) and (variable != vertical_dim) and (variable in ds):
            ds_copy = ds.copy()
            ds_copy = ds_copy[[variable]].dropna('case', how='all')
            plot_list.append(time_depth_plot(ds_copy, variable, vertical_dim, ylim, levels, cmap, vmin, vmax, log_cbar))
...
def generate_full_depth_average_plots(ds, variable_dict, vertical_dim='z_t', ylim=(1000, 0), levels=20, cmap='inferno', vmin=None, vmax=None, log_cbar=False):
    ...
    for variable in variable_dict.keys():
        if (vertical_dim in ds[variable].dims) and (variable != vertical_dim) and (variable in ds):
            ds_copy = ds.copy()
            ds_copy = ds_copy[[variable]].dropna('case', how='all')
            ds_copy[variable] = ds_copy[variable].weighted(ds.dz.dropna('case', how='all')).mean(dim='z_t', keep_attrs=True)
            if 'z_i' in ds_copy:
